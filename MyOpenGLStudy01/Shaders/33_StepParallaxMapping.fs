...
vec2 ParallaxMapping(vec2 texCoords,vec3 viewDir)
{
	...
	float currrentDepthMapValue=texture(parallaxMap,currentTexCoords).r;
	
	while(currentLayerDepth<currrentDepthMapValue)
	{
		currentTexCoords-=deltaTexCoords;
		currrentDepthMapValue=texture(parallaxMap,currentTexCoords).r;
