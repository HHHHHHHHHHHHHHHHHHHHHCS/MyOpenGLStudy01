...
void main()
{
	...
	else if(light.direction.w==1)
	{
		...
		attenuation=clamp(1-(1./(light.constant+light.linear*distance+light.quadratic*(distance*distance))),0.,1.);
	}
	else
	{
	...
	if(light.direction.w==2)
	{
		...
		attenuation=clamp(1-((theta-light.outerCutOff)/epsilon),0.,1.);
	}
