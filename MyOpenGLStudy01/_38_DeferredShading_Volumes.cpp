...
unsigned int _38_DeferredShading_Volumes::cubeVAO = 0;
unsigned int _38_DeferredShading_Volumes::quadVAO = 0;

int _38_DeferredShading_Volumes::DoMain()
{
	...
	//--------------------------
	Shader geometryPassShader{"38_GeometryPass"};
	Shader lightingPassShader{"38_DeferredLighting"};
	Shader lightBoxShader{"38_LightBox"};
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		geometryPassShader.SetMat4("projection", camera.GetProjectionMat4());
		geometryPassShader.SetMat4("view", camera.GetViewMat4());
		...
		for (unsigned int i = 0; i < lightPositions.size(); ++i)
		{
			...
			lightingPassShader.SetVec3(lightName + "Position", lightPositions[i]);
			lightingPassShader.SetVec3(lightName + "Color", lightColors[i]);

			const float constant = 1.0; // 我们只用知道这是1.0 并且写死在shader里面
...
void _38_DeferredShading_Volumes::BindCubeVAO()
{
	float vertices[] = {
		// back face
...
void _38_DeferredShading_Volumes::RenderCube()
{
	//render Cube
	glBindVertexArray(cubeVAO);
...
void _38_DeferredShading_Volumes::BindQuadVAO()
{
	float quadVertices[] = {
...
void _38_DeferredShading_Volumes::RenderQuad()
{
	glBindVertexArray(quadVAO);
